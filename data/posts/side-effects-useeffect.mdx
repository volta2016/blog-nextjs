...
**"La prioridad máxima de una aplicación" es reaccionar ante las acciones del usuario y proporcionar una respuesta visual rápida y efectiva, esta respuesta visual implica realizar cambios sobre nuestra interfaz y para hacerlo sabemos que tenemos que provocar un renderizado que al final terminara de ACTUALIZAR EL DOM, pero esto no es lo único que debe que hacer una aplicación**
...
a continuación y como consecuencia de esta busquedad hay varias tareas adicionales, que nuestra App
...
acción principal, para poderlos gestionarlos correctamente react nos proporciona el Hook de **useEffect "hook para efectos secundarios"**
...
O puede ser al revés su orden, el resultado final sería el mismo y tendría implicaciones externas a la propia función que los define, mientras que un renderizado debería afectar sólo o al nodo que se renderiza o cualquiera de sus hijos un **side effects** puede afectar a elementos externos al propio DOM -> como son el document title, la location href o la propia red que utilizamos para la llamada a la API
...
- <span className="list-icon">✅ </span> Detectar las acciones -> en que se divide
  esta tarea, podemos considerar una acción como el mínimo de conjunto de operaciones
  ...
  son simple operaciones, pero si que producen un resultado relevante, eso sería
  la diferencia entre una operación y una acción como tal que puede incluir una o
  varias operaciones.
...
👀 Lo más importante que hay que entender que elegir cuál es la acción principal, implica tomar
...
en base a lo que nosotross consideramos más prioritario como en todo tenemos que tomar una decisión
...
Vamos a ver como podemos conseguir que valor de este contador
cambie, el titulo de pestaña para indicar, aquí es donde tenemos que comenzar a analizar.
Tenemos que comenzar a pensar en esa tarea, cual sería la tarea incrementar ese contador
y de esa tarea tenemos varias acciones.

<br />

- <span className="list-icon">✅ </span> este setCount sería una acción produce algún
  resultado relevante -> si actualiza la interfaz
...
Por lo tanto aquí tenemos una tarea que se divide en 2 acciones, ahora cual es la acción principal que terminamos aquí, por un lado tenemos el setCount que es actualizar el state se renderizar en pantalla. Aquí tenemos la respuesta visual más inmediata del usuario. Donde el usuario va a centrar su foco de atención, el **document.title** no tiene mucho sentido si actualizo la app, actualiza la pestaña una vez si quito el setCount y a partir de aquí no tiene mucho sentido, entonces podemos decir claramente que de las 2, la acción principal seria **setCount** y document.title = count + 1 sería un **side effect.**

Sabiendo que es un side effects cual es problema de gestionarlo de esta manera, y ¿ por qué? no debemos hacerlo. Esto se ve claramente cuando comenzamos a añadir mas cosas a esta aplicación
...
como veo aquí lo que estoy haciendo es traerme toda esta acción principal, que evidentemente
...
y todos los demás que tenga asociado se ejecutan siempre detrás y eso es algo que nos puede inducir a error.
...
en algún punto se nos olvida hacer esta pequeña modificación, lo que pasa con esto es que tenemos una App con
...
termina de hacer todo, lo cual no tiene sentido. Podemos priorizar la acción principal dejar que
se produzca el renderizado, luego posteriormente cuando acabe el renderizado ya realizar todos
esos sideEffects porque no nos importa ni el orden en que se ejecuten, ni nos importa exactamente
el orden en que lo haga después de la acción principal y ante de la siguiente nos sirve perfectamente
y da igual si es en medio segundo ante o medio segundo después, no hay ningún problema y para
...
- <span className="list-icon">✅ </span> <b>
    es una función -> **se ejecutará siempre después del renderizado y que
    tendrá los valores actualizados
  </b>
...
ahora lo que conseguimos siempre que cambie el estado de count
...
si lo hacemos con tanto con un **+1** o con un **+2,** siempre que cambie el **state de count,** da igual que lo hagamos desde un botón, desde otro o inclusive desde un tercero, provocaremos un **side effects** que consiga exactamente esto
...
Lo que acabamos de hacer es vincular, este count a un side effects y nos da igual donde se ejecuta este count, nos da igual que si lo seteamos 1 2 o 7 veces da igual, no hay que preocuparse por esos side effects, vamos hacer un console.log(count) entendiéndolo como un pequeños side effects, no se habla de sideEffects porque no debería haber un console.log en producción, pero si ciertamente sería un pequeño side effects
...
lo cierto es que estamos definiendo una función pero no la estamos ejecutando, si te fijas le estamos pasando a useEffect una función
**no le estamos pasando su ejecución**
entonces lo que va a pasar:

<br />

**1.** Primero renderiza este console log esta ante del useEffect
...
**3.** Posteriormente seguirá haciendo todo el render o finalmente retornara un valor
...
Cuando retorne se pintara en nuestra pantalla por eso aparece en la **UI, 0 +1,+2,+3** el renderizado ya ha terminado.

<br />

Los siguiente que hace React cuando termina el renderizado, es venir y decir durante este renderizado hay que ejecutar algún useEffect? la respuesta es que si, entonces ejecuta este useEffect ante que se produzca el siguiente renderizado y de esta manera podemos verlo en consola.
...
**useEffect se ejecuta de forma asíncrona es decir, "no es inmediatamente" después del render de forma síncrona, si no que es de forma asíncrona y que implica que no podemos saber el momento exactamente cuando se ejecuta, podemos aproximarlo por que sabes que es después del render, pero no sabemos si es 10ms, 12ms etc.**

No es como **useLayaput** que ese si se ejecuta después del render forma síncrona, esto no nos importa, pero tiene sentido a la hora de hablar de métricas, podemos ver que hay un pequeño delay entre que se ejecuta y termina useEffect.
